...
public class Recommendor {
	...
	public static void main(String[] args) {
		...
		try{
		  ...
		  int NUMBER_OF_USER_RECOMMENDATIONS = 1;
		  int RECOMMENDATION_BAR = 5;
		  FileOutputStream fos = new FileOutputStream("E:\\CourseWork\\Second Semester\\Semantic Web Mining\\DataSet\\Recommendations_"+NUMBER_OF_USER_RECOMMENDATIONS+"_"+RECOMMENDATION_BAR+".csv",true);
		  fos.write(("USER ,MOVIES\n").getBytes());
		  for(int i=0;i<users.size();i++){
		    fos.write((users.get(i)+1+",").getBytes());
		  	...
		  	Collections.sort(l, new Comparator<Similarity>(){
				...
				//Un-comment this to make the nearest neighbour file
				/*
		  	int NEAREST_NEIGHBOURS=3;
		    FileOutputStream fos2 = new FileOutputStream("E:\\CourseWork\\Second Semester\\Semantic Web Mining\\DataSet\\UserSimilaritySorted_"+NEAREST_NEIGHBOURS+".csv",true);
		  	fos2.write((users.get(i)+1+",").getBytes());
		  	...
		  	*/
		  	for(int j =0;j<l.size() && j<NUMBER_OF_USER_RECOMMENDATIONS;j++){
				  //System.out.print(l.get(j).user+"::"+l.get(j).cosSimilarity+"->");
					int a = users.get(i);
					int b = l.get(j).user;
					for(int k=0;k<MOVIE_COUNT;k++){
						if(coll[k][b]>RECOMMENDATION_BAR){
							fos.write((hInv.get(k)+",").getBytes());
						}
					}
		  	}
		  	fos.write("\n".getBytes());
